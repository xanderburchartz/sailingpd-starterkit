...
|--------|-------|-------------|
| v0.1.3 | 2026-08-19 | **Dashboard & menu** (onderdeel van Starter Kit-release v1.1.0):<br>- **Configureerbaar prestatiepaneel** als standaarddashboard: alle SPD-velden, vier stijlen (SPD-Default, Modern, Raymarine, B&G), drie kleurmodes (dag / zwart-wit / nacht-rood) en drie sjablonen (Beginner / Gevorderd / Expert). Per tegel kies je veld, meter/getal/balk en grootte.<br>- **Trimadvies als coachingtegel** en doel-vs-werkelijk-paren in elke stijl.<br>- **Legacy-dashboard** (`legacy.html`) voor oude toestellen (bijv. iPad 2 / Safari 9).<br>- **Menu-redesign** met thumbnails + SPD-logo; het overzichtsmenu is de standaard startpagina.<br>- **Signal K** als eenvoudige route toe te voegen; **basismodus** verbergt de geavanceerde webserver-opties.<br>- `install.bat`/`install.sh` vervallen — de wizard installeert het menu nu zelf (exe-first).<br>- Gestructureerde testronden met bugfixes (zie `TESTRAPPORT.md`, `BACKLOG.md`). |
